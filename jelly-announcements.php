...
<head>
    ...
    <title><?php echo $is_sweet ? 'Team Announcements' : 'Team Announcements'; ?> • <?php echo pbj_hub_label('messages'); ?> • ilovepbj ops</title>
    ...
    <style>
        ...
        .post { border-left: 5px solid <?php echo $is_sweet ? '#E55163' : '#1A2A44'; ?>; border-radius: 0 14px 14px 0; padding: 14px 16px; margin-bottom: 12px; <?php if ($is_sweet): ?>background: #FFFBF8;<?php else: ?>background: #FAF8F5;<?php endif; ?> }
        .post.pinned { border-left-color: #E8A838; <?php if ($is_sweet): ?>background: #FFF9F0;<?php else: ?>background: #F7F3EA;<?php endif; ?> }
        .post-title { font-size: 1.2rem; margin: 0 0 6px; }
        .post-meta { font-size: 0.9rem; opacity: 0.7; margin-bottom: 8px; }
        .post-body { font-size: 1.05rem; line-height: 1.45; white-space: pre-wrap; margin-bottom: 10px; }
        .badge { display: inline-block; border-radius: 999px; padding: 3px 10px; font-size: 0.78rem; background: <?php echo $is_sweet ? '#FFF5F6' : '#EEF2F8'; ?>; margin-right: 6px; }
        .badge.pin { background: #FFF3D6; color: #8A5A00; }
...
<body>
    <div class="header">
        ...
        <h1><?php echo $is_sweet ? 'Team Announcements' : 'Team Announcements'; ?></h1>
        <p class="subtitle"><?php echo $is_sweet ? 'Restaurant-wide updates for everyone' : 'Restaurant-wide updates from management'; ?></p>
    </div>
    <div class="content">
        <div class="sync-pill syncing" id="sync-pill"><span class="dot"></span><span id="sync-pill-text">Connecting…</span></div>

        <input type="search" class="search" id="search" placeholder="<?php echo $is_sweet ? 'Search announcements…' : 'Search announcements…'; ?>">
        <div class="filters" id="filters">
            <button type="button" class="chip active" data-filter="all"><?php echo $is_sweet ? 'All' : 'All'; ?></button>
            <button type="button" class="chip" data-filter="pinned"><?php echo $is_sweet ? 'Pinned' : 'Pinned'; ?></button>
        </div>
        <div id="feed"></div>
        ...
        <div class="card" id="compose-card">
            <h2 id="form-heading"><?php echo $is_sweet ? 'Post announcement' : 'Post announcement'; ?></h2>
            <div class="edit-banner" id="edit-banner"><?php echo $is_sweet ? 'Editing an announcement — save when ready, or cancel ✏️' : 'Editing announcement — save to update, or cancel.'; ?></div>
            <form id="post-form">
                <input type="hidden" id="f-edit-id" value="">
                <div class="field"><label><?php echo $is_sweet ? 'Title' : 'Title'; ?></label><input id="f-title" required placeholder="<?php echo $is_sweet ? 'What should everyone know?' : 'Announcement title'; ?>"></div>
                <div class="field"><label><?php echo $is_sweet ? 'Message' : 'Message'; ?></label><textarea id="f-body" required placeholder="<?php echo $is_sweet ? 'Details, dates, who it affects…' : 'Details…'; ?>"></textarea></div>
                <div class="field-row">
                    <div class="field">
                    ...
                    <div class="field"><label><?php echo $is_sweet ? 'Pin?' : 'Pin?'; ?></label>
                        <select id="f-pin"><option value="0"><?php echo $is_sweet ? 'No' : 'No'; ?></option><option value="1"><?php echo $is_sweet ? 'Yes — keep on top' : 'Yes — keep on top'; ?></option></select>
                    </div>
                </div>
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn btn-primary" id="submit-btn"><?php echo $is_sweet ? 'Post to the jar 📢' : 'Post announcement'; ?></button>
                </div>
            </form>
        </div>
    ...
    <script>
    (function () {
        var KEY = 'pbj_jelly_announcements_v1';
        var SHARED_KEY = 'jelly_announcements_v1';
        var isSweet = <?php echo $is_sweet ? 'true' : 'false'; ?>;
            function canP(key) {
                if (window.PbjPerms && window.PbjPerms.loaded) return window.PbjPerms.can(key);
                return true;
            }
            function applyAnnPerms() {
                if (window.PbjPerms) window.PbjPerms.applyDom();
                var canView = canP('messages.announcements.view') || canP('messages.announcements.post');
                var canPost = canP('messages.announcements.post');
                if (!canView) {
        ...
        function clearForm() {
            ...
            document.getElementById('f-pin').value = '0';
            document.getElementById('edit-banner').classList.remove('show');
            document.getElementById('cancel-edit').style.display = 'none';
            document.getElementById('form-heading').textContent = isSweet ? 'Post announcement' : 'Post announcement';
            document.getElementById('submit-btn').textContent = isSweet ? 'Post to the jar 📢' : 'Post announcement';
            setAuthor(userName);
        }
        ...
        function startEdit(p) {
            ...
            document.getElementById('f-pin').value = p.pinned ? '1' : '0';
            document.getElementById('edit-banner').classList.add('show');
            document.getElementById('cancel-edit').style.display = '';
            document.getElementById('form-heading').textContent = isSweet ? 'Edit announcement' : 'Edit announcement';
        ...
        /** Newest first; pinned stay above unpinned */
        ...
        function filteredPosts() {
            ...
            if (filter === 'pinned') posts = posts.filter(function (p) { return p.pinned; });
            if (searchQ) {
                var q = searchQ.toLowerCase();
                posts = posts.filter(function (p) {
                    return [p.title, p.body, p.author].join(' ').toLowerCase().indexOf(q) !== -1;
                });
            }
            return posts;
        }
        ...
        function render() {
            ...
            if (!total) {
                ...
                root.innerHTML = '<div class="card empty">' + (isSweet
                    ? (searchQ ? 'No matches — try another search 🔍' : 'No announcements yet — post one below 📢')
                    : (searchQ ? 'No matching announcements.' : 'No announcements yet — post one below.')) + '</div>';
                return;
            }
            ...
            root.innerHTML = slice.map(function (p) {
                return '<div class="post' + (p.pinned ? ' pinned' : '') + '" id="post-' + J.esc(p.id) + '">' +
                    (p.pinned ? '<span class="badge pin">' + (isSweet ? '📌 Pinned' : 'Pinned') + '</span>' : '') +
                    '<h3 class="post-title">' + J.esc(p.title) + '</h3>' +
                    '<div class="post-meta">' + J.esc(p.author || 'Team') + ' · ' + J.esc(J.when(p.at)) + '</div>' +
                    ...
                    '<button type="button" class="btn btn-small btn-ghost" data-act="pin" data-id="' + J.esc(p.id) + '">' + (p.pinned ? (isSweet ? 'Unpin' : 'Unpin') : (isSweet ? 'Pin' : 'Pin')) + '</button>' +
        ...
        function showNewestOnFeed(postId) {
            filter = 'all';
            searchQ = '';
            page = 0; // new posts land on page 1
        ...
        document.getElementById('post-form').addEventListener('submit', function (e) {
                if (!canP('messages.announcements.post')) { e.preventDefault(); return; }
            e.preventDefault();
            ...
            var pinned = document.getElementById('f-pin').value === '1';
            ...
            if (editId) {
                ...
                persist(true, isSweet ? 'Posted to the jar 📢' : 'Posted');
            }
            clearForm();
        ...
        document.getElementById('feed').addEventListener('click', function (e) {
            ...
            if (btn.dataset.act === 'pin') {
                p.pinned = !p.pinned;
                persist(true, p.pinned ? (isSweet ? 'Pinned 📌' : 'Pinned') : (isSweet ? 'Unpinned' : 'Unpinned'));
                render();
                return;
            }
            if (btn.dataset.act === 'del') {
                if (!confirm(isSweet ? 'Remove this announcement?' : 'Remove this announcement?')) return;
        ...
        render();
            if (window.PbjPerms && window.PbjPerms.ready) window.PbjPerms.ready.then(applyAnnPerms);
            document.addEventListener('pbj-perms-ready', applyAnnPerms);
    })();
    </script>
</body>
</html>
