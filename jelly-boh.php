...
$user_name = $_SESSION['user_name'] ?? $_SESSION['name'] ?? 'BOH';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?php echo $is_sweet ? 'BOH Updates' : 'BOH Updates'; ?> • <?php echo pbj_hub_label('messages'); ?> • ilovepbj ops</title>
    ...
    <style>
        ...
        .post { border-left: 5px solid #E57373; border-radius: 0 14px 14px 0; padding: 14px 16px; margin-bottom: 12px; <?php if ($is_sweet): ?>background: #FFFBF8;<?php else: ?>background: #FAF8F5;<?php endif; ?> }
        ...
        .badge { display: inline-block; border-radius: 999px; padding: 3px 10px; font-size: 0.78rem; background: #FDECEA; color: #8B2E2E; margin-right: 6px; }
        ...
        .sync-pill { display: inline-flex; align-items: center; gap: 6px; border-radius: 999px; padding: 6px 12px; font-size: 0.88rem; margin-bottom: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); <?php if ($is_sweet): ?>background: #FFF5F6; color: #E55163; border: 1px solid #F3C5CC;<?php else: ?>background: #EEF2F8; color: #1A2A44; border: 1px solid #C5D0DE;<?php endif; ?> }
...
<body>
    <div class="header">
        ...
        <h1><?php echo $is_sweet ? 'BOH Updates' : 'BOH Updates'; ?></h1>
        <p class="subtitle"><?php echo $is_sweet ? 'Kitchen channel for line & prep notes' : 'Back-of-house kitchen notes'; ?></p>
    </div>
    <div class="content">
        <div class="sync-pill syncing" id="sync-pill"><span class="dot"></span><span id="sync-pill-text">Connecting…</span></div>

        <div class="card">
            <h2 id="form-heading"><?php echo $is_sweet ? 'Post to BOH' : 'Post to BOH'; ?></h2>
            <p class="hint"><?php echo $is_sweet ? '86s, prep gaps, equipment quirks, ticket fire — keep the line in the loop 🔥' : '86s, prep gaps, equipment notes, ticket fire.'; ?></p>
            <div class="edit-banner" id="edit-banner"><?php echo $is_sweet ? 'Editing a BOH update — save when ready, or cancel ✏️' : 'Editing BOH update — save to update, or cancel.'; ?></div>
            <form id="post-form">
                <input type="hidden" id="f-edit-id" value="">
                <div class="field"><label><?php echo $is_sweet ? 'Title' : 'Title'; ?></label><input id="f-title" required placeholder="<?php echo $is_sweet ? 'e.g. 86 tomato jam · fryer slow' : 'Short title'; ?>"></div>
                <div class="field"><label><?php echo $is_sweet ? 'Update' : 'Update'; ?></label><textarea id="f-body" required placeholder="<?php echo $is_sweet ? 'What’s happening on the line…' : 'Details…'; ?>"></textarea></div>
                <div class="field-row">
                    <div class="field"><label><?php echo $is_sweet ? 'Tag' : 'Tag'; ?></label>
                        <select id="f-tag">
                            <option value="general"><?php echo $is_sweet ? 'General' : 'General'; ?></option>
                            <option value="86">86</option>
                            <option value="prep"><?php echo $is_sweet ? 'Prep' : 'Prep'; ?></option>
                            <option value="equipment"><?php echo $is_sweet ? 'Equipment' : 'Equipment'; ?></option>
                            <option value="safety"><?php echo $is_sweet ? 'Safety / temps' : 'Safety / temps'; ?></option>
                        </select>
                    </div>
                    <div class="field">
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn btn-primary" id="submit-btn"><?php echo $is_sweet ? 'Post to BOH 🔥' : 'Post to BOH'; ?></button>
                </div>
            </form>
        </div>

        <input type="search" class="search" id="search" placeholder="<?php echo $is_sweet ? 'Search BOH updates…' : 'Search BOH updates…'; ?>">
        <div class="filters" id="filters">
            <button type="button" class="chip active" data-filter="all"><?php echo $is_sweet ? 'All' : 'All'; ?></button>
            <button type="button" class="chip" data-filter="86">86</button>
            <button type="button" class="chip" data-filter="prep"><?php echo $is_sweet ? 'Prep' : 'Prep'; ?></button>
            <button type="button" class="chip" data-filter="equipment"><?php echo $is_sweet ? 'Equipment' : 'Equipment'; ?></button>
            <button type="button" class="chip" data-filter="safety"><?php echo $is_sweet ? 'Safety' : 'Safety'; ?></button>
        </div>
        <div id="feed"></div>
        <div class="actions-bar">
            <a href="/messages/foh" class="btn btn-secondary"><?php echo $is_sweet ? 'FOH channel' : 'FOH channel'; ?></a>
    ...
    <script>
    (function () {
        var KEY = 'pbj_jelly_boh_v1';
        var SHARED_KEY = 'jelly_boh_v1';
        var isSweet = <?php echo $is_sweet ? 'true' : 'false'; ?>;
            function canP(key) {
                if (window.PbjPerms && window.PbjPerms.loaded) return window.PbjPerms.can(key);
                return true;
            }
            function applyBohUPerms() {
                if (window.PbjPerms) window.PbjPerms.applyDom();
                var canView = canP('messages.boh_updates.view') || canP('messages.boh_updates.post');
                var canPost = canP('messages.boh_updates.post');
                if (!canView) {
        ...
        function tagLabel(t) {
            var map = {
                ...
                '86': '86',
                prep: isSweet ? 'Prep' : 'Prep',
                equipment: isSweet ? 'Equipment' : 'Equipment',
                safety: isSweet ? 'Safety' : 'Safety'
            };
            return map[t] || t;
        }

        function setAuthor(name) {
            var n = name || userName || 'BOH';
        ...
        function clearForm() {
            ...
            document.getElementById('form-heading').textContent = isSweet ? 'Post to BOH' : 'Post to BOH';
            document.getElementById('submit-btn').textContent = isSweet ? 'Post to BOH 🔥' : 'Post to BOH';
            setAuthor(userName);
        }
        ...
        function startEdit(p) {
            ...
            document.getElementById('form-heading').textContent = isSweet ? 'Edit BOH update' : 'Edit BOH update';
        ...
        function render() {
            ...
            if (!posts.length) {
                root.innerHTML = '<div class="card empty">' + (isSweet
                    ? (searchQ ? 'No matches — try another search 🔍' : 'BOH channel is quiet — post the first note 🔥')
                    : (searchQ ? 'No matching updates.' : 'No BOH updates yet.')) + '</div>';
                return;
            }
            root.innerHTML = posts.map(function (p) {
                return '<div class="post"><span class="badge">' + J.esc(tagLabel(p.tag)) + '</span>' +
                    ...
                    '<div class="post-meta">' + J.esc(p.author || 'BOH') + ' · ' + J.esc(J.when(p.at)) + '</div>' +
        ...
        document.getElementById('post-form').addEventListener('submit', function (e) {
                if (!canP('messages.boh_updates.post')) { e.preventDefault(); return; }
            e.preventDefault();
            var title = document.getElementById('f-title').value.trim();
            var body = document.getElementById('f-body').value.trim();
            if (!title || !body) return;
            var tag = document.getElementById('f-tag').value || 'general';
            var author = J.getAuthorValue('f-author-sel', 'f-author', userName || 'BOH');
        ...
        document.getElementById('feed').addEventListener('click', function (e) {
            ...
            if (btn.dataset.act === 'del') {
                if (!confirm(isSweet ? 'Remove this BOH update?' : 'Remove this BOH update?')) return;
        ...
        render();
            if (window.PbjPerms && window.PbjPerms.ready) window.PbjPerms.ready.then(applyBohUPerms);
            document.addEventListener('pbj-perms-ready', applyBohUPerms);
    })();
    </script>
</body>
</html>
